                ...
                <table class="table table-striped table-sm mb-0">
                    ...
                    @foreach($daily as $row)
                        @php
                            ...
                            $persianDate = \Morilog\Jalali\Jalalian::fromCarbon(\Carbon\Carbon::parse($row['d']))->format('Y/m/d');

                            // اگر از پکیج jenssegers/date استفاده می‌کنید، خط بالا را با خط زیر جایگزین کنید:
                            // $persianDate = \Carbon\Carbon::parse($row['d'])->locale('fa')->isoFormat('YYYY/MM/DD');
                        @endphp
                        <tr>
                            <td class="text-monospace" dir="ltr">{{ $persianDate }}</td>
