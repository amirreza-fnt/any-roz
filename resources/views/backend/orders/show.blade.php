                ...
                <div class="inv-products-wrap">
                    ...
                    @foreach ($order->items as $line)
                        <div class="inv-line">
                            <div>
                                @php
                                    // اولویت با تصویر خودِ ردیف سفارش است
                                    $imageUrl = $line->image_url;

                                    // اگر تصویر ردیف نبود، سعی کن از محصول اصلی بگیرد (اگر رابطه لود شده باشد)
                                    if (empty($imageUrl) && $line->product) {
                                        $imageUrl = $line->product->image_url ?? $line->product->primary_image_url ?? null;
                                    }
                                @endphp
                                @if (!empty($imageUrl))
                                    <img src="{{ $imageUrl }}" alt="{{ $line->product_name }}" class="inv-thumb">
                                @else
