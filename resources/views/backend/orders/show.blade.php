        ...
        <div class="no-print inv-toolbar">
            ...
            <div class="d-flex flex-wrap gap-2 align-items-center">
                ...
                @if ($order->sent_to_supply && $order->sent_to_supply_at)
                    <span class="badge badge-light border text-dark small">آخرین ارسال: {{ $order->sent_to_supply_at ? \App\Support\JalaliCalendar::formatShamsiDateTime($order->sent_to_supply_at) : '—' }}</span>
                @endif
            </div>
        </div>
        ...
        <div id="invoice-print" class="invoice-sheet mb-5 {{ $hideFinancials ? 'is-supply-view' : '' }}">
            <div class="invoice-hero">
                ...
                <div class="sub d-flex flex-wrap gap-3">
                    ...
                    <span>تاریخ: <strong>{{ \App\Support\JalaliCalendar::formatShamsiDateTime($order->created_at) }}</strong></span>
                    ...
                    <div class="inv-card">
                        ...
                        <div class="inv-kv">
                            ...
                            @if ($order->payment_date)
                                <div class="small text-muted">زمان پرداخت: {{ \App\Support\JalaliCalendar::formatShamsiDateTime($order->payment_date) }}</div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="timeline mt-3">
                        ...
                        @foreach ($order->histories as $h)
                            <div class="timeline-item">
                                <div class="small text-muted">{{ \App\Support\JalaliCalendar::formatShamsiDateTime($h->created_at) }} @if($h->admin) — {{ $h->admin->full_name }} @elseif($h->user) — {{ $h->user->name }} @endif</div>
