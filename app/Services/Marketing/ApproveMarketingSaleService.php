...
class ApproveMarketingSaleService
{
    ...
    private function createOrderForSale(MarketingSale $sale): Order
    {
        ...
        $orderNumber = $sale->isColleagueSale()
            ? $this->uniqueOrderNumber('WHL-')
            : $this->uniqueOrderNumber('MKT-');

        $billing = $this->orderBillingData($sale);
        ...
        $order = Order::create([
            ...
            'shipping_method' => $billing['shipping_method'],
            'shipping_address' => $billing['shipping_address'],
            'shipping_city' => $billing['shipping_city'],
            'shipping_state' => $billing['shipping_state'],
            'shipping_postal_code' => $billing['shipping_postal_code'],
            'shipping_recipient_name' => $billing['shipping_recipient_name'],
    ...
    private function orderBillingData(MarketingSale $sale): array
    {
        if ($sale->isColleagueSale()) {
            $colleague = $sale->colleague;

            return [
                'shipping_method' => 'خرید کلی همکار',
                'shipping_address' => $colleague?->address ?: '—',
                'shipping_city' => $colleague?->city?->name ?? '—',
                'shipping_state' => $colleague?->province?->name ?? '—',
                'shipping_postal_code' => $colleague?->postal_code ?: '0000000000',
                'shipping_recipient_name' => $colleague?->full_name ?? 'همکار',
                'shipping_phone' => $colleague?->phone ?? ($sale->buyer_phone ?: '—'),
            ];
        }
    ...
    private function composeOrderNotes(MarketingSale $sale): string
    {
        ...
        if ($sale->isColleagueSale()) {
            $colleague = $sale->colleague;
            $parts[] = 'خرید کلی (همکار): '.($colleague?->full_name ?? ('#'.$sale->marketer_id));
            if ($colleague?->store_name) {
                $parts[] = 'نام فروشگاه / محل: '.$colleague->store_name;
            }
            $parts[] = 'شناسهٔ پیش‌فاکتور خرید کلی: '.$sale->id;
        } else {
            if ($sale->notes) {
    ...
    private function uniqueOrderNumber(string $prefix = 'MKT-'): string
    {
        do {
            $num = $prefix.now()->format('Ymd').'-'.strtoupper(Str::random(5));
