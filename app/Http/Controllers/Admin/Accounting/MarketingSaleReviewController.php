...
class MarketingSaleReviewController extends Controller
{
    public function index(Request $request)
    {
        ...
        $base = MarketingSale::query()
            ->where('sale_type', MarketingSale::SALE_TYPE_MARKETER)
        ...
        $counts = [
            'pending' => MarketingSale::query()->where('sale_type', MarketingSale::SALE_TYPE_MARKETER)->where('status', MarketingSale::STATUS_PENDING)->count(),
            'approved' => MarketingSale::query()->where('sale_type', MarketingSale::SALE_TYPE_MARKETER)->where('status', MarketingSale::STATUS_APPROVED)->count(),
            'rejected' => MarketingSale::query()->where('sale_type', MarketingSale::SALE_TYPE_MARKETER)->where('status', MarketingSale::STATUS_REJECTED)->count(),
        ];
